...
class cRZString : public cIGZString, public std::string
{
public:
	cRZString();
	...
	cRZString(cIGZString const* src);
    cRZString(const cRZString& src, uint32_t a3, uint32_t a4);
	virtual ~cRZString();
...
public:
	...
	bool operator>  (char const* other) const;
	bool operator== (char const* other) const;
    virtual void SprintfVaList(const char *fmt, char *dst);

protected:
	uint32_t refCount;
};

//static_assert(sizeof(cRZString) == 12);
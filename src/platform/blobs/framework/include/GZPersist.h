...
struct cGZPersistDBSegment : public cIGZPersistDBSegment {
    IUNKNOWN_COMMON_DEFS
    bool CloseRecord(cIGZPersistDBRecord *pRecord) ;
...
struct cGZPersistDBSerialRecord : public cGZPersistDBRecord,
    ...
    const cGZPersistResourceKey &GetKey() ;
...
struct cGZDBReadRecordPackedFile : public cGZDBRecordPackedFile {
    ...
    uint32_t GetSize() ;
    void SetSize(uint32_t uint32) ;
    uint32_t GetPosition() ;
    ~cGZDBReadRecordPackedFile() ;
    void SetKey(const cGZPersistResourceKey &key) ;
    bool GetFieldVoid(void *data, uint32_t len) ;
    bool SetFieldVoid(const void *pVoid, uint32_t uint32) ;
    void Flush() ;
    bool GetError() const ;
    virtual void SeekAbsolute(uint32_t pos);
    virtual void SeekRelative(int32_t pos);
...
struct cGZPersistDBSegmentPackedFile : public cGZPersistDBSegment,
    ...
    void SetPath(const cIGZString &path) ;
    bool Open(bool a2, bool a3) ;
    ...
    bool DoOpenRecordForWrite(cGZPersistResourceKey const &key,
                              cIGZPersistDBRecord **ppRecord);
    bool CloseRecord(cIGZPersistDBRecord *pRecord) ;
    bool ReadFileSpan(void *pBuffer, uint32_t nSize, uint32_t nOffset) ;
    bool WriteFileSpan(const void *data, uint32_t size, uint32_t pos) ;
